document.addEventListener('DOMContentLoaded', () => {
    ...
    function preloadAssets() {
        ...
        const imageLoads = ['images/question.png', 'images/拉杆.png', ...allImages].map(src => new Promise(resolve => {
            const img = new Image();
            img.onload = resolve;
            img.onerror = () => {
        ...
        const audioLoad = new Promise(resolve => {
            if (spinSound.readyState >= 4) return resolve();
            spinSound.addEventListener('canplaythrough', resolve, { once: true });
            spinSound.addEventListener('error', resolve, { once: true });
            spinSound.load();
        });

        Promise.all([...imageLoads, audioLoad]).then(() => {
            isReady = true;
            stopLights();
            spinButton.disabled = false;
            spinButton.textContent = '开始';
        });
    }
